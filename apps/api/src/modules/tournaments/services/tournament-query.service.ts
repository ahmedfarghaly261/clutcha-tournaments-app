...
const organizerTournamentSelect = {
  id: true,
  organizerId: true,
  name: true,
  slug: true,
...
const organizerTournamentDetailSelect = {
  ...organizerTournamentSelect,
  onlineConfiguration: {
    select: {
      id: true,
      serverRegion: true,
...
export type OrganizerTournamentDetailRecord = Prisma.TournamentGetPayload<{
  select: typeof organizerTournamentDetailSelect;
}>;
...
export class TournamentQueryService {
  ...
  async getOrganizerTournamentDetailsRecord(
    organizerId: string,
    tournamentId: string,
  ): Promise<OrganizerTournamentDetailRecord> {
    const tournament = await this.databaseService.client.tournament.findFirst({
      where: {
        id: tournamentId,
        organizerId,
      },
      select: organizerTournamentDetailSelect,
    });
  ...
  async listOrganizerTournaments(
    ...
    const [items, totalItems] = await this.databaseService.client.$transaction([
      this.databaseService.client.tournament.findMany({
        ...
        skip: (page - 1) * limit,
        take: limit,
        select: organizerTournamentSelect,
      }),
